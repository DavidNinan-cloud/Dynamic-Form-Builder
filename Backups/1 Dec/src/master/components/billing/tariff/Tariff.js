...
//importing the TariffModal form
...
//importing the asynchronous function from TariffService file
import {
  fetchAllTariff,
  deleteTariffById,
  autoSearchTariff,
...
function Tariff() {
  ...
  const filterData = () => {
    ...
    //     selectedObj.hasOwnProperty("tariff") &&
    //     selectedObj.tariff !== ""
    //   ) {
    //     searchTariff.searchString = selectedObj.tariff;
    //     console.log("if block's populateTable");
    //     populateTable(searchTariff);
    //   }
    // } catch (err) {
  ...
  const handleChange = (autoSearchString) => {
    ...
    if (autoSearchString !== "") {
      ...
      autoSearchTariff(autoSearchString)
  ...
  const autoSelectedValue = (value) => {
    ...
    if (value === null) {
      ...
      searchValue = value.TariffInfo;
    }
  };

  //event listener function for edit icon
  function editRow(TariffInfo) {
    setEdit(true);
    console.log(TariffInfo);
    console.log(TariffInfo.Id);
    setIdValue(TariffInfo.Id);
  ...
  function deleteRecord() {
    ...
    deleteTariffById(deleteId)
      .then((response) => {
        console.log(response);
        if (response.data.statusCode === 200) {
          populateTable();
          deleteAlert(response.data.result);
        }
    ...
    <>
      <div className="w-full grid pt-10 mt-8 md:rounded-md px-6">
        <div className="flex justify-center text-xl">
          <h1 className="text-gray-700  lg:hidden">Tariff</h1>
        </div>
        {/* search dropdown */}
