...
//importing the PaymentModal form
...
//importing the asynchronous function from PaymentService file
import {
  fetchAllPayment,
  deletePaymentById,
  autoSearchPayment,
} from "../../../services/billing/paymentType/PaymentTypeService";
...
function Payment() {
  ...
  const filterData = () => {
    ...
    //     selectedObj.hasOwnProperty("paymentType") &&
    //     selectedObj.paymentType !== ""
    //   ) {
    //     searchPayment.searchString = selectedObj.paymentType;
    //     console.log("if block's populateTable");
    //     populateTable(searchPayment);
    //   }
    // } catch (err) {
  ...
  const handleChange = (autoSearchString) => {
    ...
    if (autoSearchString !== "") {
      autoSearchPayment(autoSearchString)
  ...
  const autoSelectedValue = (value) => {
    ...
    if (value === null) {
      ...
      searchValue = value.PaymentInfo;
    }
  };

  //event listener function for edit icon
  function editRow(PaymentInfo) {
    setEdit(true);
    console.log(PaymentInfo);
    console.log(PaymentInfo.Id);
    setIdValue(PaymentInfo.Id);
    ...
    <>
      <div className="w-full grid pt-10 mt-8 md:rounded-md px-6">
        <div className="flex justify-center text-xl">
          <h1 className="text-gray-700  lg:hidden">Payment Type</h1>
        </div>
        <div className="flex items-center gap-4">
          <h1 className="w-48 text-xl text-gray-700  hidden lg:block">
            Payment Type
          </h1>
          {/* search dropdown */}
