...
import AddIpdAllergiesModal from "../emrcomponentsmodals/AddIpdAllegiesModal";
import IpdAllergiesTable from "../emrtables/IpdAllergiesTable";

const IpdAllergies = (props) => {
  let result = props.allergies;
  ...
  function editRow(allergyInfo, index) {
    console.log("Edit Info", allergyInfo, index);
    setIsEdit(true);
    setEditInfo(allergyInfo);
    setDataId(index);
    setOpen(true);
  }
  ...
  const validationSchema = yup.object().shape({
    allergies: yup
      .string()
      .nullable()
      .notRequired("Please Add Allergies"),
  });
    ...
    <>
        ...
        >
          <CardContent>
            <div className="flex justify-between py-2 bg-lime-50 -mt-2">
              <div className="text-sm font-semibold pl-2">Allergies</div>
              <div>
                {props.allergies !== null && props.allergies.length > 0 ? (
                  <button
                ...
                >
                  <Tooltip title="Add Allergies" placement="left-start" arrow>
      ...
      >
        ...
        >
          ...
          <IpdAllergiesTable data={details} />
        </Box>
      </Modal>
    </>
  );
