...
function GroupModal(props) {
  ...
  const { mutate: postGroup } = useMutation(addNewGroup, {
    onSuccess: (res) => {
      const result = {
  ...
  const { status } = useQuery(
    ...
    () => {
      if (props.idValue && openPut !== true) {
        return getGroupById(props.idValue);
      }
    },

    {
      ...
      onSuccess: (data) => {
        ...
        let resetObj = {
          groupDescription: data.data.result.groupDescription,
          groupName: data.data.result.groupName,
