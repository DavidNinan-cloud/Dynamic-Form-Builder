...
import {
  fetchAllModalityMaster,
  autoSearchModalityMaster,
  deleteModalityMasterById,
...
import ModalModalityMaster from "./ModalModalityMaster";
...
export default function ModalityMaster() {
  ...
  useEffect(() => {
    ...
    fetchAllModalityMaster(defaultParams)
      .then((response) => {
  ...
  const handleChange = (autoSearchString) => {
    if (autoSearchString !== "") {
      ...
      autoSearchModalityMaster(autoSearchString)
  ...
  const autoSelectedValue = (value) => {
    ...
    if (value === null) {
      ...
      searchValue = value.Modality;
    }
  };

  //EDIT API
  const editRow = (modalitymaster) => {
    setEdit(true);
    console.log(modalitymaster);
    console.log(modalitymaster.Id);
    setIdValue(modalitymaster.Id);
    handleOpen();
  };
  ...
  function deleteRecord() {
    ...
    deleteModalityMasterById(deleteId)
        .then((response) => {
  ...
  return (
    <>
      <div className="w-full grid pt-10 px-6 mt-8 md:rounded-md">
          ...
          <div className="grid grid-cols-6 xl:grid-cols-6 w-full gap-2 items-center ">
            <div className="w-full col-span-3 xl:col-span-2">
              <SearchBar
                ...
                placeholder="Search by Modality Code/Name"
                dataArray={options}
        ...
        spinner === false ? (
          <CommonMasterTable
            tableApiFunc={fetchAllModalityMaster}
            searchString={searchString}
