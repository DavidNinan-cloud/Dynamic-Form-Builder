...
export default function TestSelectionCommonMasterTable(props) {
  const [checkedRecollect, setCheckeRecollect] = React.useState(false);
  ...
  const {
    ...
    setParams,
    params,
    selectedObjCheckCollected,
    handelOpenRecollectModal,
  } = props;
  ...
  const handleAddUpdate = () => {
    ...
    setParams({
      investigationId: selectedObj.InvestigationId,
      isCultureTest: selectedObj.IsCultureTest,
      isTemplate: selectedObj.IsTemplate,
    });
    console.log("params", params);
    // let params = {
    //   orderTestId: selectedObj.LabWorkOrderTestId,
    //   isCultureTest: selectedObj.IsCultureTest,
    //   isTemplate: selectedObj.IsTemplate,
    // };
    // navigate("/lims/pathology/reportDetails", {
    //   state: params,
    // });
  };
  ...
  const handleChangeRecollect = (event) => {
    console.log("event.target.checked", event.target.checked);
    setCheckeRecollect(event.target.checked);
  };
  ...
  const handleClick = (event, row, id, index) => {
    ...
    console.log(Number(selected.toString()));
  };
                  ...
                  .map((row, index) => {
                    {
                      ...
                      return (
                        <TableRow
                          ...
                          hover
                          onClick={(event) =>
                            selectedObjCheckCollected.SampleStatus ===
                              "Sample Collected" &&
                            handleClick(event, row, row.Id, index)
                          }
                          disabled={
                            selectedObjCheckCollected.SampleStatus ===
                            "Not Collected"
                              ...
                              >
                                {editAction ? (
                                  <button
                                    ...
                                    type="button"
                                    className={`text-blue-500 underline p-1 rounded-md ${
                                      selectedObjCheckCollected.SampleStatus ===
                                      "Sample Collected"
                                        ? `cursor-pointer`
                                        : `cursor-not-allowed`
                                ...
                                {/* <FormControlLabel
                                  control={
                                    <Checkbox
                                      checked={checkedRecollect}
                                      onChange={handleChangeRecollect}
                                      size="small"
                                    />
                                  }
                                  size="small"
                                ...
                                <span
                                  className="text-orange-500 underline p-1 rounded-md cursor-pointer"
                                  onClick={handelOpenRecollectModal}
                                >
                                  Recollect/Reprocess
                                </span>
                          ...
                          {headers &&
                            headers.map((header, i) => (
                              <TableCell
                                ...
                                // }}
                              >
                                {header.toLowerCase() === "teststatus"
                                  ? (row[header] === "Pending" && (
                                      <div className="text-orange-500">
                                        Pending
                                      </div>
                                    )) ||
                                    (row[header] === "Sample Collected" && (
                                      <div className="text-green-500">
                                        Sample Collected
                                      </div>
                                    )) ||
                                    (row[header] === "Authorized" && (
