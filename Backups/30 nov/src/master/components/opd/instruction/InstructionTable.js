...
export default function InstructionTable(props) {
  ...
  const callAgainTableApi = (recordsNeeded) => {
    console.log("callAgainTableApi", defaultParams);
    ...
    tableApiFunc(defaultParams)
      ...
      .then((res) => {
        ...
        res.result.forEach((jsonString) => {
          ...
          instructionList.push(jsonObject);
        });

        let incomingData = instructionList;
  ...
  const handlePageChange = (event, newPage) => {
    ...
    console.log("handle PageChanges calleed");
                  ...
                  <TableRow className="bg-gray-100">
                    ...
                    >
                      <TableSortLabel
                        active={false} //arrow for sorting
                        direction={
                          orderBy === "instructionCode" ? order : "asc"
                        }
                        onClick={createSortHandler("instructionCode")}
                      >
                        <span className="text-gray-600 font-bold whitespace-nowrap">
                          Instruction Code
                        </span>
                        {orderBy === "instructionCode" ? (
                          <Box component="span" sx={visuallyHidden}>
                            {order === "desc"
                              ? "sorted descending"
                              : "sorted ascending"}
                    ...
                    >
                      <TableSortLabel
                        active={false} //arrow for sorting
                        direction={orderBy === "instruction" ? order : "asc"}
                        onClick={createSortHandler("instruction")}
                      >
                        <span className="text-gray-600 font-bold whitespace-nowrap">
                        ...
                        .map((instruction) => {
                          ...
                          return (
                            <TableRow>
                              ...
                              <TableCell className="flex space-y-1">
                                {instruction.instructionLocal}
                              </TableCell>
                              ...
                              //   gap: "5px",
                              //   width: "175px",
                              // }}   //for bottom line straight
                              >
                                {instruction.units &&
                                  instruction.units.map((unit) => (
                                    <>
                                      <span className="flex flex-nowrap w-full justify-center border border-gray-600 text-center rounded px-2 text-gray-600">
                                        {unit.name}
                                      </span>
                                    </>
                                  ))}
                              ...
                              <TableCell>
                                {instruction && instruction.status === true ? (
                                  <button className="px-6 text-green-600 border border-green-700 rounded">
                                    Active
                                  </button>
                                ) : (
                                  <button className="px-4 text-red-500 border border-red-600 rounded">
                                    InActive
                                  </button>
                                )}
                              </TableCell>
