...
import EnquiryVisibilityModal from "../PurchaseEnquiry/EnquiryVisibilityModal";
...
export default function purchaseEnquiryTable(props) {
  console.log("Enquiry Table Called!!!!!");
  ...
  console.log("Table enquiryId Is", enquiryId);
const[enquiryInfo,setEnquiryInfo]=React.useState("")
  const [visiblityModal, setVisibilityModal] = React.useState(false);
  //handleOpen function opens the modal form
  const handleVisibilityOpen = () => setVisibilityModal(true);
  //handelClose function closes the modal form
  const handleVisibilityClose = () => setVisibilityModal(false);
          ...
          <Paper sx={{ width: "100%", my: 2 }}>
            ...
            >
              <Table size="small">
                    ...
                    <>
                      {stableSort(dataResult, getComparator(order, orderBy))
                        ...
                        .map((enquiryData,index) => {
                          console.log("Table DataResult is: ", enquiryId);
                          return (
                            <TableRow
                              className="h-10"
                              key={index.id}
                              tabIndex={-1}
                              onClick={() => handleClick(index, enquiryData)}
                              style={{
                                backgroundColor: rowIndex === index ? "#FFF" : "",
                              }}
                           >
                              <TableCell>
                                <a
                              ...
                              <TableCell className="h-10 space-y-2 space-x-2 grid grid-cols-1">
                                {enquiryData.suppliers.map((suppliers) => (
                                  <span
                                    className="gap-2
                              ...
                              <TableCell>
                                {enquiryData &&
                                enquiryData.quotationReceived === true ? (
                                  <button className="w-24 px-6 text-green-600 rounded">
                                    yes
                                  </button>
                                ) : (
                                  <button className="w-24 px-4 text-gray-700 rounded">
                                    No
                                  </button>
                                )}
                              </TableCell>
                ...
                <EnquiryVisibilityModal
                  enquiryId={enquiryId}
                  visiblityModal={visiblityModal}
                  setVisibilityModal={setVisibilityModal}
                  handleVisibilityOpen={handleVisibilityOpen}
                  handleVisibilityClose={handleVisibilityClose}
