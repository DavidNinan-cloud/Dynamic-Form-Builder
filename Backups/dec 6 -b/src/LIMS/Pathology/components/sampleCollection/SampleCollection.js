...
const SampleCollection = () => {
  ...
  const onSubmitTableData = (data) => {
    ...
    console.log("filteredArr2.length", filteredArr2.length);
    ...
    console.log(
      "filteredArr2.length>0 ? index + Number(filteredArr2.length) : index",
      filteredArr2.length > 0 ? "" + filteredArr2.length : ""
    );
    ...
    if (filteredArr2.length > 0) {
      filteredArr.map((item, index) => {
        myObj = {
          agencyName:
            item.AgencyName && item.IsOutSourced
              ? {
          ...
          test: {
            id: Number(
              dataResultTest
                .map(
                  (itemTest) =>
                    itemTest.TestName === item.TestName && itemTest.testId
                )
                .filter((itemFilter) => itemFilter !== false)
                .toString()
            ),
          },
          // test:
          //   filteredArr2.length > 0
          //     ? arrId[index + filteredArr2.length + 1]
          //     : arrId[index],
          // test: {
          //   id: arrId[index + 1],
          // },
    ...
    let result = finalArr.filter(
      (item) => item.technician !== null && item.AgencyName !== null
    );
        ...
        <div className="flex flex-col justify-center">
          <div className="grid grid-cols-2 gap-2 my-2">
            ...
            {dataResultTest && dataResultTest.length > 0 && (
              <div className="col-span-2 p-2 border bg-white rounded-md h-auto">
                {/* {spinnerTest ? (
                  <div className="grid justify-center">
                    <LoadingSpinner />
                  </div>
