...
export default function ResultEntryTable(props) {
  ...
  React.useEffect(() => {
    if (patchArr && patchArr.length > 0) {
      ...
      console.log("patch reset arr", arr);
  ...
  React.useEffect(() => {
    if (reportEntryDetails?.parameterslist.every((item) => item.reportValue)) {
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      >
        <form onMouseLeave={handleSubmit(onSubmitResultEntryTable)}>
          {reportEntryDetails?.parameterslist && (
