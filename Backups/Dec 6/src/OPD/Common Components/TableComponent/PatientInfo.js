...
export default function PatientInfo(props) {
  console.log("PatientInfo", props.dataResult);
  const {
    tableApiFunc,
    searchString,
    dataResult,
  ...
  const handleEditForm = (editId) => {
    //Param for Navgation
    const params = { patientId: editId["Patient Id"] };
    navigate("/opd/patientregistrationform", { state: params });
  };

  //Handle View
  const handleView = (viewId) => {
    console.log("View Id", viewId["Patient Id"]);
    //Param for Navgation
    const params = { patientId: viewId["Patient Id"], isView: true };
    navigate("/opd/patientregistrationform", { state: params });
  };

  const handleEMR = (rowDetails) => {
    console.log("rowDetails", rowDetails["visitNumber"]);

    //Param for Navgation
    let params;
    params = {
      patientId: rowDetails["Patient Id"],
      visitId: rowDetails["visitId"],
      visitNumber: rowDetails["visitNumber"],
      // visitDate: rowDetails["visitDate"],
      status: rowDetails["visitStatus"],
    };
  ...
  //table start
  return (
    <div className="lg:w-[100%] mx-auto md:w-[90%] mt-4">
      <Box sx={{ width: "100%" }}>
                            ...
                            {props.data.privileges.length > 0 ? (
                              <TableCell>
                                <div className="flex justify-evenly">
                                  ...
                                  {row.visitStatus !== null &&
                                  row.visitStatus.toLowerCase() ===
                                    "completed" ? (
                                    <button
                                      type="button"
                                      onClick={() => {
