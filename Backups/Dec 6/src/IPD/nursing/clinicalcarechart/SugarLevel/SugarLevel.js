...
function SugarLevel() {
  ...
  function updateSugarLevelRmo() {
    ...
    updateSugarLevelByRmo(finalData)
      .then((response) => {
        console.log("RMO Updated OBJ IS " + response);
        if (response.data.statusCode === 200) {
          successAlert(response.data.message);
          populateTable(patientVisitId);
        }
  ...
  function updateSugarLevelNurse() {
    ...
    updateSugarLevelByNurse(finalData)
      .then((response) => {
        console.log("Nurse Updated OBJ IS " + response);
        if (response.data.statusCode === 200) {
          successAlert(response.data.message);
          populateTable(patientVisitId);
        }
  ...
  let inj = watch("injection");
  let tab = watch("tablet");
  let bsl = watch("bsl");
  return (
    <div>
