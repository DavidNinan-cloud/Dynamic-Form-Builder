...
export default function SymptomsModal(props) {
  ...
  function addRecord() {
    ...
    postSymptoms(finalData);
  }
  //useMutation hook for the implementation of post request data saving
  const { mutate: postSymptoms } = useMutation(addNewSymptoms, {
    onSuccess: (res) => {
      const result = {
  ...
  const { status } = useQuery(
    ["Symptoms", props.idValue],
    () => {
      if (props.idValue && openPut !== true) {
        return getSymptomsById(props.idValue);
      }
    },
    {
      enabled: props.edit,
      ...
      onSuccess: (data) => {
        ...
        let resetObj = {
          ...
          symptomCode: data.data.result.symptomCode,
          symptom: data.data.result.symptom,
