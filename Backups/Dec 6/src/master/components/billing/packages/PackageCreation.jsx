      ...
      getPackageDetails(71).then((response) => response.data).then((res) => {
        ...
        if(resultList[0].isPackageHealthPlan){
          ...
          defalutobj = {
            ...
            "activityDays": "5",
            "isAuto": true,
            "isPackageHealthPlan": false,
            "packageStartDate": "2022-12-06",
            "packageEndDate": "2022-12-06",
            "serviceCode": "New Pck 211",
            "serviceName": "New Package",
            "group": 
              {id: 2, value: 2, label: "IPD Charges"},
            "subGroup":  
            {id: 10, value: 10, label: "IPD Subgroup"},
            "active": true,
            "units": [
                 
{id: 14, value: 14, label: "City Hospital"},
                
{id: 9, value: 9, label: "Alliance Multispeciality Hospital "}
            ],
            "tariff":[
              {id: 8, value: 8, label: "Ageon Religare"},
              {id: 1, value: 1, label: "Hospital"}],
            "adminChargesApplicable": true,
            "adminChargesInAmount": "100",
            "adminChargesInPercent": 0,
            "odpApplicableTo": true,
            "doctorShareApplicable": true,
            "tdsPercent": 9,
            "isPackage": true,
            "discountApplicable": true,
            "isOutSourced": true,
            "rateEditAllowed": true,
            "packageGroups" : [
              {id: 2, value: 2, label: "IPD Charges"},
              {id: 3, value: 3, label: "OPD Charges"}
            ]
          }
          // onEditPackage(resultList[0])
          onEditPackage(defalutobj)
        }
        // setComingData(resultList[0]);
        setComingData(defalutobj);
      });
    },[])

    const onEditPackage = (data) => {
      ...
      if(data.adminChargesInAmount > 0){
        setValue('adminAmount',data.adminChargesInAmount, { shouldValidate: true,})
        setValue('adminChargeIn',{id: 0, label: "Rupees", value: 0}, { shouldValidate: true,})
      }else{
        setValue('adminAmount',data.adminChargesInPercent, { shouldValidate: true,})
        setValue('adminChargeIn',{id: 1, label: "Percent", value: 1 }, { shouldValidate: true,})
      }


      console.log('setDateValue1',data.packageStartDate)
      console.log('setDateValue2',data.packageEndDate)
      setValue('dob',data.packageStartDate)
      setValue('toDate',data.packageEndDate)
    ...
    const onEditHealthPlan = (data) => {
      ...
      serviceDetailsArr.forEach((field, i) => {
        if(field == 'packageGroups'){
            console.log('packageGroups',data[field])
        }
        setValue(field, data[field], { shouldValidate: true,});
      });
      ...
      // for - package rate auto
        ...
        if(isPackageHealthPlan){
          ...
          // for - healthplan -false 
            ...
            finalDataObj.packageGroupRequestDtoList = fnsetgroupRatesArr(data,finalDataObj)
          // groupwise
          if(data.packageRateType == 0){
            finalDataObj.packageServicesRequestDtoList = []
    ...
    const fnsetgroupRatesArr = (data,finalDataObj) => {
