...
import DepartmentTable from "../organization/DepartmentTable";
...
export default function Department() {
  ...
  const handleChange = (autoSearchString) => {
    if (autoSearchString !== "") {
      ...
      autoSearchDepartments(autoSearchString)
  ...
  function populateTable() {
    ...
    fetchAllDepartments(obj)
      ...
      .then((res) => {
        ...
        departmentList.statusCode = res.statusCode;
  ...
  const callTableDataApi = () => {
    showSpinner(true);
    showRecordWarning(false);
    fetchAllDepartments(stringSearchObj)
      .then((response) => {
        console.log(response.data);
        setCount(response.data.count);
        return response.data;
  ...
  return (
    <div className="w-full grid pt-10 px-6 mt-8 md:rounded-md">
      <div className="flex justify-center text-xl">
        <h1 className=" text-gray-700 font-Poppins lg:hidden ">Department</h1>
      </div>
      ...
      spinner === false ? (
        <DepartmentTable
          tableApiFunc={fetchAllDepartments}
          searchString={searchString}
