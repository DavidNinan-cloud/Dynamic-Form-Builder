...
import {
  ...
  getTestResultType,
  getRadiologistList,
} from "../services/RadioTemplateService";
...
function RadioTemplateModal(props) {
  ...
  //axios get request to have all the list of test result type
  React.useEffect(() => {
    getTestResultType().then((response) => {
      console.log("The test result types are", response);
      setTestResultType(response.data.result);
    });

    getRadiologistList().then((response) => {
      console.log("The radiologist list is ", response);
              ...
              <fieldset className="grid border border-gray-300 text-left w-full  lg:mx-auto lg:px-4 md:mr-0 py-1 rounded mt-8 lg:m-2">
                ...
                >
                  <div className="grid justify-center grid-cols-2 lg:grid-cols-3 gap-2">
                    ...
                    {/* Test Result dropdown field */}
                    <div className="w-full">
                      <DropdownField
                        control={control}
                        placeholder="Select TestResult"
                        name="testResult"
                        error={errors.testResult}
                        isSearchable={false}
                        dataArray={testResultType}
                      />
                    </div>

                    {/* Radiologist dropdown field */}
                    <div className="w-full">
                      <DropdownField
                        control={control}
...
export default RadioTemplateModal;
