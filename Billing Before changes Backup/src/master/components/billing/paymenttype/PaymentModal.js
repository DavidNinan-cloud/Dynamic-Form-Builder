...
function PaymentModal(props) {
  console.log("This is PaymentType Modal component");
  //Yup schema
  ...
  const onSubmitDataHandler = (data) => {
   ...
   function addRecord() {
    ...
    if (props.countClick === 0) {
      ...
      postPayment(finalData);
    }
  }

   //useMutation hook for the implementation of post request data saving
   const { mutate: postPayment } = useMutation(addNewPayment, {
    onSuccess: (res) => {
      const result = {
  ...
  const { status } = useQuery(
    ["paymentInfo", props.idValue],
    //to avoid the automatic firing of the query. Because PaymentModal is a child component of Payment.js
    () => {
      if (props.idValue && openPut !== true) {
        return getPaymentTypeById(props.idValue);
      }
    },
    {
      enabled: props.edit,
      ...
      onSuccess: (data) => {
        ...
        let resetObj = {
          paymentType: data.data.result.paymentType,
          ...
          <Box sx={ModalStyle}>
            ...
            <div className="row">
              <fieldset className="border border-gray-300 text-left w-full   py-8 rounded mt-8 lg:m-2 ">
                <legend className="px-2 font-bold text-gray-700 ml-2">
                  Add Payment Type
                </legend>
                <form
                ...
                >
                  <div className="py-2 grid grid-cols-2 w-full px-3 gap-2">
                  ...
                  <div className="flex gap-4 w-full items-center justify-end px-2">
