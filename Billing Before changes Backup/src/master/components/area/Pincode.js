...
//importing the PincodeModal form
...
//importing the asynchronous function from PincodeService file
import {
  fetchAllPincodes,
  deletePincodeById,
  autoSearchPincode,
...
export default function Pincode() {
  ...
  useEffect(() => {
    ...
    let defaultParams = {
      page: 0,
      size: rowsPerPage,
      searchPin: searchString,
    };
    fetchAllPincodes(defaultParams)
      .then((response) => {
  ...
  function editRow(pincodeInfo) {
    setEdit(true);
    console.log(pincodeInfo);
    console.log(pincodeInfo.Id);
    setIdValue(pincodeInfo.Id);
    ...
    <>
      <div className="w-full grid pt-10 px-6 mt-8 md:rounded-md">
        <div className="flex justify-center text-xl">
          <h1 className="text-black font-Poppins lg:hidden ">Pincode</h1>
        </div>
