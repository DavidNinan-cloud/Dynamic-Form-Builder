...
function RefundModal(props) {
  ...
  const schema = yup.object().shape({
    refundAmount: yup
      .string()
      .required("Required")
      .min(5, "Add at least 5 Alphanumeric block Code")
      .matches(
        /^[A-Za-z0-9]+$/,
  ...
  const { mutate: postRefund } = useMutation(addNewRefund, {
    onSuccess: (res) => {
      const result = {
   ...
   const { status } = useQuery(
    ...
    () => {
      if (props.idValue && openPut !== true) {
        return getRefundById(props.idValue);
      }
    },
    {
      enabled: props.edit,
      ...
      onSuccess: (data) => {
        ...
        let resetObj = {
          ...
          refundAmount: data.data.result.refundAmount,
          remarks: data.data.result.remarks,
